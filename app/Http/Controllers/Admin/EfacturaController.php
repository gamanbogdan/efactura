...
class EfacturaController extends Controller
{
        ...
        if(request()->ajax()) 
        {
            ...
            ->addColumn('Is_fcn', function($row){ 
                ...
                elseif($row->is_fcn===null) {
                    ...
                    $is_fcn = "<i class='fas fa-check text-secondary'> </i>";
                }
                
                return $is_fcn;
            })
            ->rawColumns([ 'Nr_factura', 'Date_created_anaf', 'Informatii_factura_Data_emitere_factura', 'Produse', 'Is_fcn'])
    ...
    public function upload(Request $request) {
            ...
            if ($status !== true) {
             throw new \Exception($status);
                ...
                try {
                    ...
                    $zip->extractTo($this->storageDestinationPathUnzip.'/'.$time_for_path);
                    $zip->close(); 


                    $path_zip = $this->storageDestinationPathUnzip.'/'.$time_for_path;
                    $dir_path_zip = opendir($path_zip);
                
                    while (false !== ($fileNameDataAnaf = readdir($dir_path_zip))) {
                        if ($fileNameDataAnaf == "." || $fileNameDataAnaf == "..") continue;

                        $dir_data_anaf  = opendir($this->storageDestinationPathUnzip.'/'.$time_for_path.'/'.$fileNameDataAnaf);
                        ...
                        while (false !== ($fileNameXML = readdir($dir_data_anaf))) {
                            ...
                            if(in_array($ext, array("xml")) && $prefix_xml!="semnatura_") {
                                ...
                                $invoicePathModel->xml_name = $prefix_xml;
                                $invoicePathModel->xml_path = $time_for_path.'/'.$fileNameDataAnaf. '/'.$fileNameXML;
                                ...
                                $invoice_details_xml = $this->get_details_from_xml($time_for_path.'/'.$fileNameDataAnaf. '/'.$fileNameXML);
        ...
        ->with('facturi','Au fost inserate '.$numar_facturi.' facturi')  
        ->with('lista_data_incarcare','Date incarcare: '. $lista_data_incarcare_anaf_string)
...
/*
        ...
        //BT-39 Subdiviziunea
        $invoice['Vanzator']['Adresa']['Subdiviziunea'] = (string) @$cac->AccountingSupplierParty->Party->PostalAddress->children($cbc_namespace)->CountrySubentity ;
        ...
        if ($cac->PaymentMeans) {
            ...
            $invoice['Instructiuni_de_plata']['Codul_tipului_instrumentului_de_plata'] = (string) @$cac->PaymentMeans->children($cbc_namespace)->PaymentMeansCode ;
            
            //BT-82 Nota privind instrumentul de plata
            $invoice['Instructiuni_de_plata']['Nota_privind_instrumentul_de_plata'] = null; 
            if(@$cac->PaymentMeans->children($cbc_namespace)->InstructionNote) {       
                $invoice['Instructiuni_de_plata']['Nota_privind_instrumentul_de_plata'] = (string) @$cac->PaymentMeans->children($cbc_namespace)->InstructionNote ;
            }
            
            //BT-83 Aviz de plata
            $invoice['Instructiuni_de_plata']['Aviz_de_plata'] = null;
            if(@$cac->PaymentMeans->children($cbc_namespace)->PaymentID) {
                $invoice['Instructiuni_de_plata']['Aviz_de_plata'] = (string) @$cac->PaymentMeans->children($cbc_namespace)->PaymentID ;
            }
            ...
            if(@$cac->PaymentMeans->CardAccount->children($cbc_namespace)->PrimaryAccountNumberID) {
                $invoice['Instructiuni_de_plata']['Numarul_contului_principal_al_cardului_de_plata'] =  (string) @$cac->PaymentMeans->CardAccount->children($cbc_namespace)->PrimaryAccountNumberID ;
            }
            
            //BT-88 Numele detinatorului cardului de plata
            $invoice['Instructiuni_de_plata']['Numele_detinatorului_cardului_de_plata'] = null; 
            if(@$cac->PaymentMeans->CardAccount->children($cbc_namespace)->HolderName) {
                $invoice['Instructiuni_de_plata']['Numele_detinatorului_cardului_de_plata'] = (string) @$cac->PaymentMeans->CardAccount->children($cbc_namespace)->HolderName ;
            }
            
            //BT-84 Identificatorul contului de plata
            $invoice['Instructiuni_de_plata']['Identificatorul_contului_de_plata'] = (string) @$cac->PaymentMeans->PayeeFinancialAccount->children($cbc_namespace)->ID;
            
            //BT-85 Numele contului de plata
            $invoice['Instructiuni_de_plata']['Numele_contului_de_plata'] = (string) @$cac->PaymentMeans->PayeeFinancialAccount->children($cbc_namespace)->Name;
            ...
            if(@$cac->PaymentMeans->PayeeFinancialAccount->FinancialInstitutionBranch) {
                ...
                $invoice['Instructiuni_de_plata']['Identificatorul_furnizorului_de_servicii_de_plata'] =  (string) @$cac->PaymentMeans->PayeeFinancialAccount->FinancialInstitutionBranch->children($cbc_namespace)->ID ;
            }
            ...
            if(@$cac->PaymentMeans->PaymentMandate->children($cbc_namespace)->ID) {
                $invoice['Instructiuni_de_plata']['Debitare_directa']['Identificatorul_referintei_mandatului'] = (string) @$cac->PaymentMeans->PaymentMandate->children($cbc_namespace)->ID ;
            }
            ...
            if(@$cac->PaymentMeans->PaymentMandate->PayerFinancialAccount) {
                $invoice['Instructiuni_de_plata']['Debitare_directa']['Identificatorul_contului_debitat'] = (string) @$cac->PaymentMeans->PaymentMandate->PayerFinancialAccount->children($cbc_namespace)->ID ;
            }
        ...
        if(@$cac->AllowanceCharge[0]) {
            ...
            $invoice['Taxarea_suplimentara']['Codul_motivului'] = (string) @$cac->AllowanceCharge[0]->children($cbc_namespace)->AllowanceChargeReasonCode;
            
            //BT-97/BT-104 Motivul
            $invoice['Taxarea_suplimentara']['Motivul'] = (string) @$cac->AllowanceCharge[0]->children($cbc_namespace)->AllowanceChargeReason;
           
            //BT-94/BT-101 Procent
            $invoice['Taxarea_suplimentara']['Procent'] = (string) @$cac->AllowanceCharge[0]->children($cbc_namespace)->MultiplierFactorNumeric;
                       
            //BT-92/BT-99 Valoare
            $invoice['Taxarea_suplimentara']['Valoare'] = (string) $cac->AllowanceCharge[0]->children($cbc_namespace)->Amount;
           
            //Codul monedei RON
            $invoice['Taxarea_suplimentara']['Codul_monedei_RON'] = (string) @$cac->AllowanceCharge[0]->children($cbc_namespace)->Amount->attributes()->currencyID;
           
            //BT-93/BT-100 Valoarea de baza
            $invoice['Taxarea_suplimentara']['Valoarea_de_baza'] = (string) @$cac->AllowanceCharge[0]->children($cbc_namespace)->BaseAmount;

            //BT-95/BT-102 Codul categoriei de TVA
            $invoice['Taxarea_suplimentara']['Codul_categoriei_de_TVA'] =  (string) @$cac->AllowanceCharge[0]->TaxCategory->children($cbc_namespace)->ID ;
            
            //BT-96/BT-103 Cota de TVA
            $invoice['Taxarea_suplimentara']['Cota_de_TVA'] =  (string) @$cac->AllowanceCharge[0]->TaxCategory->children($cbc_namespace)->Percent;
            ...
            if(@$cac->AllowanceCharge[0]->TaxCategory->TaxScheme) {
                $invoice['Taxarea_suplimentara']['Identificatorul_schemei_VAT'] =  (string) @$cac->AllowanceCharge[0]->TaxCategory->TaxScheme->children($cbc_namespace)->ID ;
        ...
        if(@$cac->AllowanceCharge[1]) {
            ...
            //BT-98/BT-105 Codul motivului
            $invoice['Deducere']['Codul_motivului'] = (string) @$cac->AllowanceCharge[1]->children($cbc_namespace)->AllowanceChargeReasonCode;
          
            //BT-97/BT-104 Motivul
            $invoice['Deducere']['Motivul'] = (string) @$cac->AllowanceCharge[1]->children($cbc_namespace)->AllowanceChargeReason;
           
            //BT-94/BT-101 Procent
            $invoice['Deducere']['Procent'] = (string) @$cac->AllowanceCharge[1]->children($cbc_namespace)->MultiplierFactorNumeric;
            
            //BT-92/BT-99 Valoare
            $invoice['Deducere']['Valoare'] = (string) $cac->AllowanceCharge[1]->children($cbc_namespace)->Amount;
            
            //Codul monedei RON
            $invoice['Deducere']['Codul_monedei_RON'] = (string) @$cac->AllowanceCharge[1]->children($cbc_namespace)->Amount->attributes()->currencyID ;
           
            //BT-93/BT-100 Valoarea de baza
            $invoice['Deducere']['Valoarea_de_baza'] = (string) @$cac->AllowanceCharge[1]->children($cbc_namespace)->BaseAmount;
            
            //BT-95/BT-102 Codul categoriei de TVA
            $invoice['Deducere']['Codul_categoriei_de_TVA'] = (string) @$cac->AllowanceCharge[1]->TaxCategory->children($cbc_namespace)->ID;
           
            //BT-96/BT-103 Cota de TVA
            $invoice['Deducere']['Cota_de_TVA'] = (string) @$cac->AllowanceCharge[1]->TaxCategory->children($cbc_namespace)->Percent;
           
            
            //Identificatorul schemei VAT
            $invoice['Deducere']['Identificatorul_schemei_VAT'] = null;
            if(@$cac->AllowanceCharge[1]->TaxCategory->TaxScheme) {
                $invoice['Deducere']['Identificatorul_schemei_VAT'] = (string) @$cac->AllowanceCharge[1]->TaxCategory->TaxScheme->children($cbc_namespace)->ID ;
